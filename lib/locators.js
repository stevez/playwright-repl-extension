 ...
 */
export function buildClickElementJS(target) {
  ...
  return `
    (() => {
      ...
      const interactive = [...document.querySelectorAll('button, a, [role="button"], input[type="submit"], input[type="button"]')];
      ...
      if (!el) return { error: 'No element found matching: ' + text };
      el.scrollIntoView({ block: 'center' });
      el.click();
      return { success: true, tag: el.tagName.toLowerCase() };
    })()
